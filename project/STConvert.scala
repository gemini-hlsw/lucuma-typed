/**
 * ScalablyTyped Copyright (C) 2023 Øyvind Raddum Berg
 *
...
import org.scalablytyped.converter.internal.{BuildInfo, InFolder, Json, constants, files, sets}
...
object STConvert {
  ...
  implicit val ReadsFlavour: Read[Flavour] =
    Read.reads(s =>
      Flavour.byName.getOrElse(s, sys.error(s"'$s' is not among ${Flavour.byName.keys}"))
    )
  ...
  implicit def ReadsSelection[T: Ordering](implicit ts: Read[Seq[T]]): Read[Selection[T]] =
    Read.reads {
  ...
  val ParseConversionOptions: OParser[Unit, Config] = {
    ...
    OParser.sequence(
      ...
      opt[os.Path]('d', "directory")
        ...
        .text(
          "Specify another directory instead of the current directory where your package.json and node_modules is"
        ),
  ...
  def main(args: Array[String]): Unit =
    OParser.parse(ParseConversionOptions, args, DefaultConfig) match {
      case Some(
        ...
        val packageJsonPath =
          c.paths.packageJson.getOrElse(sys.error(s"$inDir does not contain package.json"))
        val nodeModulesPath =
          c.paths.node_modules.getOrElse(sys.error(s"$inDir does not contain node_modules"))
        ...
        val projectSource: Option[LibTsSource.FromFolder] =
          if (includeProject)
            Some(LibTsSource.FromFolder(InFolder(inDir), TsIdentLibrary(inDir.last)))
          else None
        ...
        val bootstrapped =
          Bootstrap.fromNodeModules(InFolder(nodeModulesPath), conversion, wantedLibs)
...
//            "ignoredModulePrefixes" -> conversion.ignoredModulePrefixes.toString,
        ...
        val Pipeline: RecPhase[LibTsSource, PublishedSbtProject] =
          RecPhase[LibTsSource]
            ...
            .next(
              new PhaseFlavour(conversion.flavourImpl,
        ...
        val results: Map[LibTsSource, PhaseRes[LibTsSource, PublishedSbtProject]] =
          sources
            .map(source =>
              source -> PhaseRunner(Pipeline, (_: LibTsSource) => logger.void, NoListener)(source)
            )
            .toMap
